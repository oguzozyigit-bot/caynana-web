...
// ✅ FINAL: Yeni sohbet başlığı = ilk user mesajı (max 15 karakter)
...
// ✅ scroll zorlamasın: sadece alttaysan takip
...
return   <div class="menu-action ${m.group ?grp-${m.group}: ""} ${m.tone ?tone-${m.tone}: ""}" data-action="${m.key}">   <div class="ico">${m.ico}</div>   <div><div>${m.label}</div><small>${m.sub}</small></div>   </div>  ;
}
...
$("loginOverlay")?.classList.add("active} else {
