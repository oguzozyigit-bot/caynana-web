(()=>{
  ...
  if(!ownNumber)return;
  let peer=null,peerOpen=false,activeCall=null,localStream=null,remoteVideo=null,localVideo=null,facing='user';
  ...
  async function getMedia(face='user'){if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia)throw new Error('Kamera desteklenmiyor');return navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true},video:{facingMode:{ideal:face}}})}
  ...
  function renderLive(remote,n){show('GÖRÜNTÜLÜ GÖRÜŞME',n,'<div class="videoStage"><video id="remoteV" autoplay playsinline></video><video id="localV" class="videoLocal" autoplay muted playsinline></video></div>','<button class="videoCam" id="switchCam">KAMERA ÇEVİR</button><button class="videoHang" id="videoHang">KAPAT</button>');remoteVideo=document.getElementById('remoteV');localVideo=document.getElementById('localV');remoteVideo.srcObject=remote;localVideo.srcObject=localStream;remoteVideo.play().catch(()=>{});localVideo.play().catch(()=>{});document.getElementById('videoHang').onclick=()=>cleanup(true);document.getElementById('switchCam').onclick=async()=>{try{facing=facing==='user'?'environment':'user';const ns=await getMedia(facing);const nt=ns.getVideoTracks()[0];const at=ns.getAudioTracks()[0];const senders=activeCall&&activeCall.peerConnection?activeCall.peerConnection.getSenders():[];const vs=senders.find(s=>s.track&&s.track.kind==='video');const as=senders.find(s=>s.track&&s.track.kind==='audio');if(vs&&nt)await vs.replaceTrack(nt);if(as&&at)await as.replaceTrack(at);const old=localStream;localStream=ns;if(localVideo)localVideo.srcObject=localStream;if(old)old.getTracks().forEach(t=>t.stop())}catch(e){}}}
