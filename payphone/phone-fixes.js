...
function fixBlackBerri(){
  ...
  document.querySelectorAll('.phoneCard').forEach(card=>{
    card.classList.remove('bb2001fix');
...
function openUniqueGame(){
  ...
  window.__uniqueGameTimer=setInterval(()=>{
    const c=$('#ugCanvas'); if(!c)return; const g=c.getContext('2d');
    g.clearRect(0,0,280,190);g.fillStyle='#173242';g.fillRect(0,0,280,190);
    phase++;
    ...
    g.fillStyle='#e3ad35';g.fillRect(x,y,14,14);g.fillStyle='#f4f7f8';g.beginPath();g.arc(ox,oy,8,0,Math.PI*2);g.fill();
